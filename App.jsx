...
// طبقة تخزين محلية (localStorage) بدل storage الخاص بالآرتيفاكتس
const storage = {
  get: async (key) => {
...
export default function TradingLedger() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await storage.get("trades");
        if (res && res.value) setTrades(JSON.parse(res.value));
      } catch (e) {
        // no data yet
      }
      try {
        const keyRes = await storage.get("finnhub_key");
  ...
  const saveApiKey = async (val) => {
    setApiKey(val);
    try {
      await storage.set("finnhub_key", val);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await storage.set("trades", JSON.stringify(trades));
      } catch (e) {
